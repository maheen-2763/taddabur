...
class StorySeeder extends Seeder
{
    ...
    {
        // ❌ Story::truncate(); / StoryChapter::truncate(); — hata diya, cascade-delete risk khatam
        ...
        foreach ($stories as $entry) {
            $prophet = Prophet::where('slug', $entry['prophet_slug'])->first();

            if (!$prophet) {
                $this->command->error("❌ Prophet '{$entry['prophet_slug']}' nahi mila — skip kiya.");
                continue;
            }

            $story = Story::updateOrCreate(
                ['slug' => $entry['story']['slug']],
                array_merge($entry['story'], ['prophet_id' => $prophet->id])
            );
            $storyCount++;

            foreach ($entry['chapters'] as $chapter) {
                StoryChapter::updateOrCreate(
                    ['story_id' => $story->id, 'order' => $chapter['order']],
                    [
                        'title'            => $chapter['title'],
                        'content'          => $chapter['content'],
                        'quran_references' => $chapter['quran_references'],
                    ]
                );
                $chapterCount++;
            }
        ...
        $this->command->info("✅ Stories seeded/updated: {$storyCount} stories, {$chapterCount} chapters");
    }
}
